		...
		<main>
			<div class="head-title">
			 ...
			 <!-- ajouter tableau ici -->
			  ...
			  <?php 
			 	if (isset($_GET["ModulAdded"])) {
					?>
					<div class="alert alert-success"><?=$_GET["ModulAdded"]?></div>
				<?php
				}
			...
			<?php
             ...
             <div class="container mt-4">
                <button class="btn btn-success mb-3" data-toggle="modal" data-target="#addUserModal">Ajouter une matière</button>
                <table class="table table-bordered">
					...
					$ModulDisplay=$cn->query("select idMat,matiere,coef,nbHeures,niveau from matiere m, niveau n where m.idNiv=n.idNiv");
					while ($ModulResult=$ModulDisplay->fetch(PDO::FETCH_OBJ)) {
						?>

					
					<tbody>
						<tr>
							<td><?=$ModulResult->idMat?></td>
							<td><?=$ModulResult->matiere?></td>
							<td><?=$ModulResult->coef?></td>
							<td><?=$ModulResult->nbHeures?></td>
							<td><?=$ModulResult->niveau?></td>
							<td>
								<div class="action-buttons">
									<button class="btn btn-warning btn-sm">modifier</button>
									<a href="deleteUsers.php?Userid=<?=$ModulResult->idMat?> "><button class="btn btn-danger btn-sm">Supprimer</button></a>
            ...
            <div class="modal-body">
                <form action="levelProcessing.php" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="coeff" class="form-control" id="coeff">
                    </div>
                    <div class="form-group">
                        <label for="nbheure">nombres d'heures</label>
                        <input type="number" name="nbheure" class="form-control" id="nbheure">
                    </div>
					...
					<div class="form-group">
                        <label for="idProf">niveau</label>
                        <select class="form-control" name="idProf" id="idProf">
                        <?php 
					
                    //recuperer tous toutes le sections
                    $displayProf = $cn->query("select * from prof");
                    while ($teacher = $displayProf->fetch(PDO::FETCH_OBJ)) {
						?>
                            <option value="<?=$teacher->idProf?>"><?=$teacher->nom?></option>
                ...
                <button  class="btn btn-primary" name="addModul">Ajouter</button>
            </div>
                </form>
            </div>
